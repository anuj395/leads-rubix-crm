...
import { fetchTasks, type TaskApiRow } from '../api/tasksApi'

// ─── Local mock data (mirrors the shape of real API responses) ────────────────

const MOCK_TASKS: TaskApiRow[] = [
  {
...
export function useTasks({
  ...
  const [rows, setRows] = useState<TaskApiRow[]>([])
    ...
    async function load() {
      try {
        if (industry_id) {
          ...
          const result = applyMockClientSide(MOCK_TASKS, {
            search,
            sortField: sort.field,
            sortOrder: sort.order,
            filters: activeFilters,
