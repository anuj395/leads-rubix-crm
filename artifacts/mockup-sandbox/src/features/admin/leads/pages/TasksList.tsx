/**
 * features/admin/leads/pages/TasksList.tsx
 *
...
const TasksListPage = () => {
  ...
  const { rows, total, loading: dataLoading, error: dataError, reload: reloadData } =
    useTasks({ industry_id, pagination, sort, search, activeFilters })
  ...
  const handleExport = useCallback(() => {
    apiRef.current?.exportDataAsCsv?.({ fileName: 'tasks-export' })
  }, [apiRef])

  const handleEdit = useCallback((row: ContactApiRow) => {
    alert(`Edit tasks: ${String(row.name ?? row.customer_name ?? row.id)}`)
  }, [])

  const handleDelete = useCallback((row: ContactApiRow) => {
    if (confirm(`Delete tasks: ${String(row.name ?? row.customer_name ?? row.id)}?`)) reloadData()
  }, [reloadData])
    ...
    <Box sx={{
      ...
      {configError && (
        <Alert severity="error" sx={{ mb: 1, flexShrink: 0 }}>
          Column config unavailable — please tasks your Super Admin to create table headers for this industry. Rendering with default columns.
        </Alert>
      )}
      {!configLoading && dbColumns.length === 0 && (
        <Alert severity="error" sx={{ mb: 1, flexShrink: 0 }}>
          No table columns found for this industry (ID: {String(industry_id ?? 'N/A')}). Please tasks your Super Admin to create the table headers.
        </Alert>
      )}
      {dataError && (
        <Alert severity="warning" sx={{ mb: 1, flexShrink: 0 }}>
          Could not load tasks from server — showing cached data. ({dataError})
        </Alert>
      )}
      ...
      {dbColumns.length > 0 && (
        <Paper elevation={0} sx={{
          ...
          <TableTopBar
            ...
            onAdd={() => alert('Open Add Task dialog')}
            onImport={() => alert('Import tasks')}
